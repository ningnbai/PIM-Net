...
class Resize(object):
    ...
    def __call__(self, image, mask):
        image = cv2.resize(image, dsize=(self.W, self.H), interpolation=cv2.INTER_NEAREST)
...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image = Image.open(self.cfg.datapath+'/rgb/'+name+'.jpg').convert('RGB')
        image = cv2.imread(self.cfg.datapath+'/rgb/'+name+'.jpg')[:,:,::-1].astype(np.float32)
        mask  = cv2.imread(self.cfg.datapath+'/mask/' +name+'.jpg', 0).astype(np.float32)
        shape = mask.shape
        ...
        if self.cfg.mode=='train':
            ...
            image, mask = self.totensor1(image, mask)
